# update-imports-advanced.ps1
# Script to update complex import paths in flattened test files
...
foreach ($file in $testFiles) {
    ...
    # 3. Special cases with '../src' patterns
    $newContent = $newContent -replace '\.\.\/src\/(components|services|hooks|context)\/', 'src/$1/'
    
    # 4. Multi-level from within nested folders
    $newContent = $newContent -replace '\.\.\/\.\.\/\.\.\/src\/(components|services|hooks|context)\/', 'src/$1/'
    
    # 5. Fix test-utils
    $newContent = $newContent -replace 'src\/test-utils', '../../test-utils'
