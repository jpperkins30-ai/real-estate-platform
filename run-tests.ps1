# run-tests.ps1 - Updated test runner for real-estate-platform with corrected directory structure
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ Component tests failed. See errors above for details." -ForegroundColor Red
    Set-Location -Path ".."
    exit $LASTEXITCODE
}
...
if ($COVERAGE_RESULT -ne 0) {
    Write-Host "❌ Coverage thresholds not met. Please improve test coverage." -ForegroundColor Red
    Set-Location -Path ".."
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ Critical component tests failed." -ForegroundColor Red
    Set-Location -Path ".."
    exit $LASTEXITCODE
}
...
npx vitest run "src/_tests_/TC201_components_multiframe*" --config ./vitest.config.ts
$CORE_TEST_RESULT = $LASTEXITCODE
if ($CORE_TEST_RESULT -ne 0) {
    Write-Host "❌ Core functionality tests failed." -ForegroundColor Red
    Set-Location -Path ".."
