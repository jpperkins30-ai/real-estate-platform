/**
 * Migrate county documents to use customId field
 * 
 * This script:
 * 1. Finds all counties in the database
 * 2. Updates each county to have a customId based on state and name
...
// Extract the state code from a county ID (e.g., "tx-harris" -> "tx")
...
async function runMigration() {
  try {
    console.log('Starting county ID migration...');
    ...
    // Create a map of state IDs to abbreviations for quick lookup
    const stateMap = {};
    ...
    for (const county of counties) {
      try {
        // Skip if already has a customId
        if (county.customId) {
          console.log(`County ${county.name} already has customId: ${county.customId}`);
          continue;
        }
        ...
        if (county.stateId && stateMap[county.stateId.toString()]) {
          ...
          const matchedState = states.find(s => 
            s.abbreviation.toLowerCase() === stateAbbr
          );
          
          // If found, update the county's stateId
          if (matchedState) {
